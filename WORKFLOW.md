...
- ⚠️ **อัพผิดสาขา = catalog ของสาขานั้นผิดทันที** (listener ดันไปทุกเครื่อง) — แก้โดยอัพไฟล์ที่ถูกต้องทับ
- **คอลัมน์ D:** แถวที่เป็น `D` / `P` / `REVIEW` จะถูกข้าม — ถ้าสินค้านั้นยังมีสต็อกใน R01 จะขึ้น badge **DEL** สีแดง (ปกติ ตั้งใจ) แต่**ยังนับใน Total SKU ตามปกติ**
...
1. **ตั้งใจ (ปกติ)** — สินค้านั้นคอลัมน์ D เป็น `D` หรือ `P` จึงถูกกรองออกจาก Product Branch Master
   → ยังสแกน + Confirm + เข้าใบปรับสต็อกได้ปกติ และ**ยังนับใน Total SKU** · DEL เป็นแค่ป้ายบอกว่า "ไม่มีชื่อในไฟล์ catalog"
...
- Total SKU = จำนวน SKU ใน **R01.102** ที่ยอดคงเหลือ **≠ 0** และ**หมวด (คอลัมน์ P) ไม่ใช่ประเภทที่ไม่ต้องนับ**
- ไม่นับ: ยอดคงเหลือ **0 พอดี** · หมวดขึ้นต้น **"11."** (อุปกรณ์สำนักงาน/ค่าใช้จ่าย/ขนส่ง) · หมวดที่มีคำว่า **DELETE**
- **ยอดคงเหลือติดลบยังต้องนับ** (คือการขายของขาด)
- toast ตอนอัพ R01 บอกจำนวนที่ตัดออกไว้แล้ว เช่น "R01.102: 5,400 รายการ · ไม่นับ 120 (หมวดไม่ใช่สินค้า)"
- สินค้าหมวดที่ไม่นับ **ยังสแกนได้ Confirm ได้ปกติ** แค่ไม่ขึ้นใน Total SKU/Progress
- badge Product Master ขึ้น "ยังไม่โหลด" ไม่ทำให้ Total SKU ผิด (ตัวเลขมาจาก R01) แต่ชื่อสินค้าจะมาจาก R01 แทน
...
| **DEL** | SKU อยู่ใน R01 แต่ไม่อยู่ใน Product Branch Master — รวมกรณีคอลัมน์ D = `D`/`P` ที่ถูกกรองออกตั้งใจ (ยังนับใน Total SKU ปกติ) |
