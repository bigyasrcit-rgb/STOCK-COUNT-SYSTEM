...
- ดึงข้อมูลล่าสุดจาก Firestore (R01, R05, scanData, Product Branch Master ของสาขานี้)
...
### ขั้นที่ 1: Product Branch Master (แยกไฟล์ต่อสาขา — ส.ค. 2026)

- เปิด **Admin Mode** (ปุ่ม 🔒 ที่ header → ใส่ PIN `22190`)
- **ต้องอัพทีละสาขา รวม 4 ครั้ง** (SRC / KKL / SSS / WH) — ไม่ใช่อัพครั้งเดียวใช้ทุกสาขาแบบเดิม
  1. สลับสาขา → 2. เข้า Admin Mode → 3. **ตรวจชื่อสาขาบนหัวจอก่อนเลือกไฟล์** → 4. อัพ → 5. ดู toast ว่าขึ้นชื่อสาขาถูก
- เก็บที่ Firestore `stock_sessions/{branch}_pm` (`SRC_pm` / `KKL_pm` / `SSS_pm` / `WH_pm`) → เครื่องอื่นในสาขาเดียวกันได้รับอัตโนมัติ
- ⚠️ **อัพผิดสาขา = catalog และ Total SKU ของสาขานั้นผิดทันที** (listener ดันไปทุกเครื่อง) — แก้โดยอัพไฟล์ที่ถูกต้องทับ
- **คอลัมน์ D ทำ 2 อย่าง:**
  - `D` / `P` → ข้ามแถวทิ้ง · ถ้าสินค้านั้นยังมีสต็อกใน R01 จะขึ้น badge **DEL** สีแดง (ปกติ ตั้งใจ) แต่**ยังนับใน Total SKU ตามปกติ**
  - `A` / `B` / `C` / `REVIEW` (ขายดีมาก / ปานกลาง / ขายได้เรื่อยๆ / รอตรวจสอบ) → **ต้องเดินไปนับเสมอ แม้ระบบจะขึ้นยอดคงเหลือ 0** เพราะเป็นของที่หมุนเร็วหรือยังไม่ได้ตรวจสอบ ต้องดูของบนชั้นจริง — ทั้ง 4 กลุ่มนี้อยู่ใน catalog ปกติ ไม่ติดแท็ก DEL
  - ค่าอื่น (ว่าง, `N`, `E`, …) → นับตามยอดคงเหลือเหมือนเดิม (มีของ = นับ · ไม่มีของ = ไม่ต้องนับ)
- ⚠️ **อัพ PBM ใหม่ = Total SKU และ Progress ของสาขานั้นเปลี่ยนทันทีทุกเครื่อง** (จะ **เพิ่มขึ้น** เพราะ A/B/C/REVIEW ที่ยอด 0 ถูกดึงเข้ามา) — คำนวณเลขที่คาดหวังใน Excel ไว้ก่อนแล้วเทียบ และแจ้งพนักงานล่วงหน้า
- ดู toast หลังอัพ: `Product Master (SRC): 5,102 รายการ · A/B/C/REVIEW 4,371 · พบราคา 4,980`
  **ถ้า `A/B/C/REVIEW` เป็น 0** = ไฟล์ไม่มีคอลัมน์ D → Total SKU จะเท่ากติกาเก่า (นับทุก SKU ที่ยอดคงเหลือ ≠ 0) ให้ตรวจไฟล์ก่อน
- สาขาที่ยังไม่อัพ → badge ขึ้น "ยังไม่โหลด" และชื่อสินค้าจะมาจาก R01 แทน (ระบบไม่ดึงไฟล์ของสาขาอื่นมาใช้)
...
- 🟢 Product Master Ready (ต้องเป็นไฟล์ของสาขานี้ — ถ้าขึ้น "ยังไม่โหลด" คือยังไม่ได้อัพ PBM ของสาขานี้)
- 🟢 R05.106 Ready
- 🟢 R01.102 Ready
...
|---|---|---|
| Product Branch Master | แยกต่อสาขา (`SRC_pm` ฯลฯ) | แยกต่อสาขา (`WH_pm`) |
...
- อัพ Product Branch Master + R05.106 (panel ที่ปกติซ่อน) — ⚠️ PM ลงที่**สาขาที่เลือกอยู่**เท่านั้น ตรวจชื่อสาขาก่อนเลือกไฟล์
...
### 🔴 สินค้าขึ้น badge DEL สีแดงเยอะผิดปกติ
สาเหตุได้ 2 อย่าง:
1. **ตั้งใจ (ปกติ)** — สินค้านั้นคอลัมน์ D เป็น `D` หรือ `P` จึงถูกกรองออกจาก Product Branch Master
   → ยังสแกน + Confirm + เข้าใบปรับสต็อกได้ปกติ และ**ยังนับใน Total SKU** ถ้ายังมีของ · DEL เป็นแค่ป้ายบอกว่า "ไม่มีชื่อในไฟล์ catalog"
2. **ไฟล์ไม่ครบ** — PBM ของสาขานั้นไม่มี SKU เหล่านั้นเลย → ทางแก้: อัพ PBM ของ**สาขานั้น**ใหม่ให้ครบ

**ปุ่มกรอง 🗑️ DEL** โชว์เฉพาะ DEL ที่**ต้องนับจริง** (ยังมีของ และหมวดเป็นสินค้าคงคลัง) — ใช้เป็นรายการเดินหาของได้เลย
ส่วน**ป้ายแดง DEL ในตาราง**ยังขึ้นครบทุกตัว รวมตัวที่ของหมดแล้ว → ถ้าอยากดูทั้งหมดให้กด "ทั้งหมด" แล้วมองหาป้ายแดงแทน

### ❓ ตัวเลขบนการ์ดสถิติหมายถึงอะไร / ทำไมบวกไม่ลงตัว
| การ์ด | หมายถึง |
|---|---|
| **Total SKU** | จำนวนที่ต้องนับทั้งหมด (= ตัวหาร Progress) |
| **Counted** | นับ + ยืนยันไปแล้วกี่ตัว — **เท่ากับตัวเลขหน้า `/` ใน Progress เสมอ** |
| **Pass** | ในจำนวนที่นับแล้ว ตรงกับระบบกี่ตัว |
| **Audit** | งานที่รอเภสัชตรวจ (WH = รอรีเช็ค) — **ตรงกับเลขบนปุ่ม Audit Verify เสมอ** |
| **Not in System** | บาร์โค้ดที่ยิงแล้วไม่พบในระบบ |

⚠️ **`Pass + Audit` จะไม่เท่ากับ `Counted` — เป็นเรื่องปกติ ไม่ใช่ระบบผิด**
เพราะ **Counted/Pass นับเฉพาะของในรายการที่ต้องนับ** ส่วน **Audit นับงานค้างทุกตัว** รวมของนอกรายการนับที่พนักงานยิงเจอบนชั้น (ซึ่งต้องให้เภสัชไปตรวจเหมือนกัน)

### ❓ จะดูว่าเหลืออะไรต้องสแกนอีกบ้าง
📋 **รายการสต็อคสินค้า → ปุ่ม ⏳ ยังไม่ได้นับ** — กด Export Excel เอาไปเดินหาของได้เลย

**จำนวนแถวในรายการ = ตัวเลขที่เหลือใน Progress พอดีเสมอ** (เช่น Progress `4,500 / 4,800` → รายการมี 300 แถว)
→ ไล่สแกนจนรายการหมด = Progress ครบ 100% พอดี ไม่ต้องเดา

ในรายการมี 2 แบบปนกัน ดูที่ป้ายสถานะในแถว:
- **รอนับ** = ยังไม่ได้สแกน → ต้องเดินไปหาของ
- **กำลังนับ** = สแกนแล้วแต่ยังไม่ได้กด Confirm → **ไม่ต้องเดินไปหาแล้ว แค่กด Confirm**

⚠️ ถ้ารายการว่างแต่ Progress ยังไม่ 100% แปลว่าข้อมูลจากเครื่องอื่นยังมาไม่ครบ — รอ sync แล้วกดปุ่มรีเฟรช

### 🔴 Total SKU เป็น 0 หรือ Progress ไม่ขยับ
**Total SKU = SKU ที่มีแถวใน R01.102 · หมวด (คอลัมน์ P) ไม่ใช่ประเภทที่ไม่ต้องนับ · และ (ยอดคงเหลือ ≠ 0 **หรือ** คอลัมน์ D ใน PBM เป็น `A`/`B`/`C`/`REVIEW`)**
- **ยอดคงเหลือ ≠ 0** → นับ (กติกาเดิม) · **ยอดคงเหลือติดลบก็นับ** (คือการขายของขาด)
- **คอลัมน์ D = `A`/`B`/`C`/`REVIEW`** → นับ **แม้ยอดคงเหลือจะเป็น 0** (สินค้าขายดี/รอตรวจสอบ ต้องไปดูของบนชั้นจริง)
- ไม่นับมีแค่: ไม่มีแถวใน R01 · หมวดขึ้นต้น **"11."** (อุปกรณ์สำนักงาน/ค่าใช้จ่าย/ขนส่ง) หรือมีคำว่า **DELETE** · หรือ (ยอดคงเหลือ 0 **และ** ไม่ใช่ A/B/C/REVIEW)
- หมวดใน R01 **ชนะทุกอย่าง** — คอลัมน์ D เป็น `A` และมีของ แต่หมวดเป็น "11." ก็ยังไม่นับ
- สินค้าที่ไม่นับ **ยังสแกนได้ Confirm ได้ปกติ** แค่ไม่ขึ้นใน Total SKU/Progress
- **Total SKU ไม่เพิ่มหลังอัพ PBM** → ดู toast ตอนอัพว่า `A/B/C/REVIEW` เป็น 0 หรือเปล่า · เป็น 0 = ไฟล์ไม่มีคอลัมน์ D → แก้โดยอัพไฟล์ที่มีคอลัมน์ D
- badge Product Master ขึ้น "ยังไม่โหลด" → สาขานั้นยังไม่ได้อัพ PBM → Total SKU จะนับตามยอดคงเหลืออย่างเดียว และชื่อสินค้าจะมาจาก R01 แทน
...
| **DEL** | SKU อยู่ใน R01 แต่ไม่อยู่ใน Product Branch Master — รวมกรณีคอลัมน์ D = `D`/`P` ที่ถูกกรองออกตั้งใจ · สแกน/Confirm ได้ปกติ และยังนับใน Total SKU ถ้ายังมีของ · **ปุ่มกรอง 🗑️ DEL โชว์เฉพาะตัวที่ต้องนับ** ส่วนป้ายแดงในตารางขึ้นครบทุกตัว |
| **A / B / C / REVIEW** | การจัดชั้นตามยอดขายในคอลัมน์ D ของ PBM (ขายดีมาก / ปานกลาง / ขายได้เรื่อยๆ / รอตรวจสอบ) — **4 กลุ่มนี้ต้องเดินไปนับแม้ระบบขึ้นยอด 0** และไม่ติดแท็ก DEL |
