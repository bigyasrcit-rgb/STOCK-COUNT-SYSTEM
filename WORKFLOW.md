...
- **คอลัมน์ D ทำ 2 อย่าง:**
  - `D` / `P` / `REVIEW` → ข้ามแถวทิ้ง · ถ้าสินค้านั้นยังมีสต็อกใน R01 จะขึ้น badge **DEL** สีแดง (ปกติ ตั้งใจ) แต่**ยังนับใน Total SKU ตามปกติ**
  - `A` / `B` / `C` (ขายดีมาก / ปานกลาง / ขายได้เรื่อยๆ) → **ต้องเดินไปนับเสมอ แม้ระบบจะขึ้นยอดคงเหลือ 0** เพราะเป็นของที่หมุนเร็ว ต้องดูของบนชั้นจริง
  - ค่าอื่น (ว่าง, `N`, `E`, …) → นับตามยอดคงเหลือเหมือนเดิม (มีของ = นับ · ไม่มีของ = ไม่ต้องนับ)
- ⚠️ **อัพ PBM ใหม่ = Total SKU และ Progress ของสาขานั้นเปลี่ยนทันทีทุกเครื่อง** (จะ **เพิ่มขึ้น** เพราะ A/B/C ที่ยอด 0 ถูกดึงเข้ามา) — คำนวณเลขที่คาดหวังใน Excel ไว้ก่อนแล้วเทียบ และแจ้งพนักงานล่วงหน้า
- ดู toast หลังอัพ: `Product Master (SRC): 5,102 รายการ · A/B/C 4,371 · พบราคา 4,980`
  **ถ้า `A/B/C` เป็น 0** = ไฟล์ไม่มีคอลัมน์ D → Total SKU จะเท่ากติกาเก่า (นับทุก SKU ที่ยอดคงเหลือ ≠ 0) ให้ตรวจไฟล์ก่อน
...
**Total SKU = SKU ที่มีแถวใน R01.102 · หมวด (คอลัมน์ P) ไม่ใช่ประเภทที่ไม่ต้องนับ · และ (ยอดคงเหลือ ≠ 0 **หรือ** คอลัมน์ D ใน PBM เป็น `A`/`B`/`C`)**
- **ยอดคงเหลือ ≠ 0** → นับ (กติกาเดิม) · **ยอดคงเหลือติดลบก็นับ** (คือการขายของขาด)
- **คอลัมน์ D = `A`/`B`/`C`** → นับ **แม้ยอดคงเหลือจะเป็น 0** (สินค้าขายดี ต้องไปดูของบนชั้นจริง)
- ไม่นับมีแค่: ไม่มีแถวใน R01 · หมวดขึ้นต้น **"11."** (อุปกรณ์สำนักงาน/ค่าใช้จ่าย/ขนส่ง) หรือมีคำว่า **DELETE** · หรือ (ยอดคงเหลือ 0 **และ** ไม่ใช่ A/B/C)
...
- **Total SKU ไม่เพิ่มหลังอัพ PBM** → ดู toast ตอนอัพว่า `A/B/C` เป็น 0 หรือเปล่า · เป็น 0 = ไฟล์ไม่มีคอลัมน์ D → แก้โดยอัพไฟล์ที่มีคอลัมน์ D
...
| **A / B / C** | การจัดชั้นตามยอดขายในคอลัมน์ D ของ PBM (ขายดีมาก / ปานกลาง / ขายได้เรื่อยๆ) — **3 กลุ่มนี้ต้องเดินไปนับแม้ระบบขึ้นยอด 0** |
